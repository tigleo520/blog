<template>
  <div>
    ...
    <el-card>
      <el-row>
        <el-col>
          <el-button type="primary" plain @click="adEdit()">添加广告位</el-button>
        </el-col>
      </el-row>
      <el-table :data="items">
        ...
        <el-table-column fixed="right" label="操作" align="center" width="120">
          <template slot-scope="scope">
            <el-button @click="adEdit(scope.row)" type="text" size="small">编辑</el-button>
...
export default {
  ...
  methods: {
    ...
    async adEdit (row) {
      ...
      if (row) {
        const res = await this.$http.get(`rest/ads/${row._id}`)
        this.edit = res.data
      }
    },
    dialogClosed () {
    ...
    async save () {
      if (this.edit._id) {
        await this.$http.put(`rest/ads/${this.edit._id}`, this.edit)
      } else {
        await this.$http.post('rest/ads', this.edit)
      }
    ...
    remove (row) {
      this.$confirm(`是否确定删除Ad'${row.title}'`, '提示', {
